    ...
    if len(titulos) != len(datas):
        ...
        for titulo, data in zip(titulos, datas):
            ...
            if data_texto == data_procurada:
                ...
                # Salvando o PDF no local especificado
                save_path = os.path.join(r'C:\Users\willi\OneDrive\Documentos\RE-FastscreenNovo\DiárioOficialDoEstado\documents', f'{diario_id}.pdf')
                with open(save_path, 'wb') as file:
                    file.write(pdf_response.content)
                
                print(f"PDF salvo em: {save_path}")
                
                # Extraindo texto do PDF salvo
                extracted_text = extract_text_from_pdf(save_path)
                
                # Salvando o texto extraído em um arquivo JSON
                json_filename = f'{diario_id}_ocr_{datetime.now().strftime("%Y%m%d")}.json'
                json_path = os.path.join(r'C:\Users\willi\OneDrive\Documentos\RE-FastscreenNovo\DiárioOficialDoEstado\documents', json_filename)
                with open(json_path, 'w', encoding='utf-8') as json_file:
                    json.dump({"text": extracted_text}, json_file, ensure_ascii=False, indent=4)
                
                print(f"OCR salvo em: {json_path}")
